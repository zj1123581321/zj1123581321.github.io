...
本文是「LLM 吞噬一切」系列的客户端篇。远端 24h 大本营怎么搭，见上一篇 [Agent 不下班：随时随地随设备的开发体系](https://mp.weixin.qq.com/s/JeiK11Ix00WmQmAJej9lqA)；硬件基座怎么选，见 [Agent 的家](https://mp.weixin.qq.com/s/I2ksdFm_jbNqoTPD_QSuuw)。

微信文章可能反爬，可以让 Agent 直接读原始 markdown：
> https://zj1123581321.com/posts/260630-agent-era-mba-setup/
...
![MBA 作为遥控台连接云端所有设备](https://zj1123581321.com/posts/260630-agent-era-mba-setup/ai-image.jpg)
...
在 Windows 上我一直重度使用 [Quicker](https://getquicker.net/)。它可以很方便地做各种自动化操作和快速启动，设计得非常直观。我一度觉得，Mac 上如果没有类似的工具，我就很难切过来。
...
![我的多屏工位：左侧 27 寸 2K 屏、中间 34 寸带鱼屏、右侧 MBA、上方还有一块共享显示器](https://zj1123581321.com/posts/260630-agent-era-mba-setup/desk-setup.jpg)
...
[BetterDisplay](https://betterdisplay.pro) 的解法是：在 2K 屏上强行开启 HDPI 模式，创建一个虚拟的高分辨率，然后缩放到 2K 屏上显示。效果类似于让 macOS 以为连接了一块高分辨率屏幕，用更高分辨率渲染后再缩放到 2K 屏上显示。
...
[eqMac](https://eqmac.app) 接管了系统的音频输出，让键盘音量键重新能控制外接音箱的音量。它本身还是个均衡器，但我用它纯粹就是为了这一个功能。

### 窗口管理三件套

| 工具 | 解决什么 |
|---|---|
| **[Stay](https://cordlessdog.com/stay/)** | 每次插拔外屏，所有窗口位置全乱。Stay 按显示器配置记忆窗口位置，插上自动归位 |
| **[AltTab](https://alt-tab-macos.netlify.app)** | macOS 原生 Cmd+Tab 功能太简陋：只显示应用图标，看不到窗口内容；而且所有屏幕的窗口混在一起。AltTab 解决两个问题：一是切换时能**实时预览**每个窗口正在做什么，不用盲猜；二是可以设置**只切换当前屏幕的窗口**，多屏工作时不会被其他屏幕的窗口干扰 |
| **[Rectangle](https://rectangleapp.com)** | 键盘快速分屏。Windows 用户的肌肉记忆，Win+方向键分屏，Rectangle 把这个体验搬到了 Mac 上 |

![AltTab：切换窗口时实时预览每个窗口的内容](https://zj1123581321.com/posts/260630-agent-era-mba-setup/alttab-preview.jpg)

这三个我觉得对任何 Mac 多屏用户来说都是刚需，不展开了。

## 三、外接键盘：用 Windows 模式 + 改键，反而更好

这一节的核心工具是 **[Karabiner-Elements](https://karabiner-elements.pqrs.org)**。
...
| **Mac 连 Mac** | **[NoMachine](https://nomachine.com)** | VNC 协议太费带宽，在网络一般的情况下卡成幻灯片。NoMachine 用的 NX 协议，带宽占用小得多，体验好很多 |
| **多台 Linux 虚拟机** | **[Royal TSX](https://royalapps.com/ts/mac)** | 支持 RDP、VNC、SSH、SFTP 多协议，可以在一个窗口里用标签页管理所有 Linux 设备 |
| **临时救急** | **[ToDesk](https://todesk.com)** | 国内穿透方便，偶尔用 |
...
![Royal TSX：一个窗口用标签页管理所有远程连接](https://zj1123581321.com/posts/260630-agent-era-mba-setup/royal-tsx-ui.png)

### 网络前提

远程管理的前提是网络互通。这块在上一篇文章里已经详细写过了，核心就是 **[Tailscale](https://tailscale.com)** 把所有设备拉进同一个虚拟局域网。这里不重复。
...
- **[Claude Code](https://docs.anthropic.com/en/docs/claude-code)**：主力 Agent，独立安装
- **[Codex](https://github.com/openai/codex)**：OpenAI 的 CLI Agent，备用
- **[OpenCode](https://opencode.ai/)**：开源方案，最底层的备用
...
[Ferdium](https://ferdium.org) 把每个网页服务"固定"成一个独立的 APP 面板，在侧边栏一键切换。本质上它是一个多标签的浏览器壳，但比你在 Chrome 里开一堆标签页要好管理得多。

![Ferdium：左侧侧边栏聚合多个服务，一键切换](https://zj1123581321.com/posts/260630-agent-era-mba-setup/ferdium-ui.jpg)

### CotEditor：Mac 上的 Notepad++

有时候你只是想打开一个文件看一眼、改两行，不需要启动 VS Code 那么重的东西。[CotEditor](https://coteditor.com) 就是这个定位：轻量、秒开、语法高亮够用。类似 Windows 上的 Notepad++。
...
- **[PixPin](https://pixpin.cn)**：截图 + 贴图 + OCR。比系统自带截图强在两个地方：一是"贴图"功能（截完图可以 Pin 在屏幕上不消失，对照着写代码很方便）；二是 OCR 文字识别。
- **[Cap](https://cap.so)**：轻量录屏工具。需要录个操作过程发给别人的时候用。

### 视频和媒体

- **[Downie](https://software.charliemonroe.net/downie)**：视频下载工具。在 Windows 上我用 IDM 下 YouTube，它通过浏览器扩展直接捕获视频流，成功率非常高，比 yt-dlp 稳定得多。Mac 上没有 IDM，Downie 是我找到的最接近的替代。
- **[IINA](https://iina.io)**：Mac 上最好的视频播放器，没有之一。
- **HandBrake / ffmpeg**：视频转码。有 GUI 需求用 [HandBrake](https://handbrake.fr)，命令行操作用 ffmpeg。

### 效率增强

- **[Maccy](https://maccy.app)**：剪贴板历史。复制过的东西都留着，随时调出来。轻量，只做这一件事。
- **[Stats](https://github.com/exelban/stats)**：菜单栏系统监控。CPU、内存、温度、网速一目了然。不需要专门开个窗口，扫一眼菜单栏就知道机器状态。

![Stats：点击菜单栏图标弹出详细的系统监控面板](https://zj1123581321.com/posts/260630-agent-era-mba-setup/stats-dashboard.png)

- **[Synergy](https://symless.com/synergy)**：一套键鼠控制多台电脑。我桌上有 MBA 和 Mac Studio 两台机器，Synergy 让我用一套键盘鼠标在两台电脑之间无缝移动，不用伸手换。
...
- **[豆包输入法](https://shurufa.doubao.com)**：AI 辅助输入。配合前面 Karabiner 的 CapsLock→Fn 映射，长按 CapsLock 就开始语音输入。在 Agent 时代，很多时候你跟 Claude Code 交流就是"说"出来比"打"出来快。另外建议把系统自带的 ABC 输入法移除，让电脑只保留豆包一个输入法，用 Shift 键切换中英文。这样你永远不用关心"当前是什么输入法"这个问题，少一层心智负担。
