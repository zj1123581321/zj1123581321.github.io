...
所以推荐买二手的 **M1 Max Mac Studio**——相比当前 M4/M5 的新机型，同预算能买到更大的内存，这才是最实际的。没必要为更新的芯片付溢价。至于选 Mac mini 还是 Mac Studio，其实没什么所谓，核心就是看内存大小。
...
Linux 路线的门槛确实会高一点，你至少需要对 SSH 有个基本概念（下一节会解释）。但它的适用范围更广——本质上**任何一台能装 Windows 的电脑都能装 Linux**，你不需要额外买新设备。

而且重申一下：**你不需要自己学教程。** 你只需要知道"我要做什么"，具体的操作让 Claude Code 通过 SSH 连上去帮你搞定。你大部分时候只是输个密码、点个确认。
...
上面这些坑我都已经踩完了，我的 tmux 配置参考放在文章末尾的附录里，让 Agent 直接读取配置到服务器即可。
...
[VS Code](https://code.visualstudio.com/)（Visual Studio Code）是微软出品的一款免费代码编辑器，可以理解成程序员的"Word"——写代码、看代码、管理项目文件都在里面。它是目前全世界最流行的代码编辑器，教程和插件极其丰富。
...
[Peer Relay](https://tailscale.com/docs/features/peer-relay) 是一个进阶功能：**你可以把自己网络里的某台有公网 IP 的设备指定为中继节点**。比起走 Tailscale 海外的公共中继站，通过你自己的公网服务器中转，延迟更低、速度更快。

具体操作很简单：阿里云和腾讯云都有 99 元一年的轻量服务器，大概 3Mbps 的带宽——写代码来回传输的数据量非常小，这个带宽完全够用。你只需要在这台服务器上安装 Tailscale 客户端并开启 Peer Relay 功能，Tailscale 就会在网络不好的时候自动通过它中转，保证基础的连接质量。以前实现类似的效果需要自己搭复杂的中转服务，现在一个配置就搞定了，非常推荐。

> 类比：DERP ≈ "走 Tailscale 的公共快递站中转，在海外，比较远"，Peer Relay ≈ "在国内买了个自己的快递中转站，近很多"。
...
| tmux | 配置见下方 | 需要 tmux 3.2+，低版本不支持 `extended-keys` 和 `allow-passthrough`。用 `tmux -V` 检查版本 |

#### tmux 配置参考（写入 `~/.tmux.conf`）
