...
> 本文是「LLM 吞噬一切」系列的开发环境篇。硬件基座怎么搭，见上一篇 [Agent 的家，AI 时代个体的硬件基座](https://mp.weixin.qq.com/s/I2ksdFm_jbNqoTPD_QSuuw)；软件层怎么搭，见 [我用 AI 长出来的那些工具](https://mp.weixin.qq.com/s/w8VnWJcUp5VkD5J-fYCUrg)。这篇聊的是：硬件和软件都就位以后，怎么让你的 Agent 24 小时在线，而你可以随时随地、用任何设备接入。
>
> 还是那句话——**作为人类，你只需要读懂这篇文章的架构逻辑。所有具体的软件安装、环境配置，把这篇文章丢给 Claude Code 或 Codex，让它去执行就行。**

微信文章可能反爬，可以让 Agent 直接读原始 markdown：
> https://zj1123581321.com/posts/260628-agent-era-dev-anywhere/

## 一、一个认知转变

在 [硬件篇](https://mp.weixin.qq.com/s/I2ksdFm_jbNqoTPD_QSuuw) 里我就提过：按 Coding Agent 当前的发展速度，我大概率不会再买高性能笔记本了。所有预算会迁移到高性能主机上。笔记本出门唯一的需求就是更大的屏幕看得更舒服——MacBook Air 这种便携续航型产品，才更符合 AI 时代编程的需求。
...
![架构拓扑图](https://zj1123581321.com/posts/260628-agent-era-dev-anywhere/architecture-diagram.png)
...
> 关于 Mac 统一内存架构在 AI 时代的独特优势，以及硬件市场的价格趋势，详见 [硬件篇](https://mp.weixin.qq.com/s/I2ksdFm_jbNqoTPD_QSuuw)。
...
- **Claude Code**：Anthropic 官方的 CLI 工具，我的主力
- **Codex**：OpenAI 的 CLI Agent
- **OpenCode**：开源方案，作为最底层的备用
- **Hermes**：Anthropic 的多 Agent 编排工具
...
![tmux 多窗口并行运行多个 Coding Agent](https://zj1123581321.com/posts/260628-agent-era-dev-anywhere/tmux-multi-agent.png)
...
VS Code 是微软出品的一款免费代码编辑器，可以理解成程序员的"Word"——写代码、看代码、管理项目文件都在里面。它是目前全世界最流行的代码编辑器，教程和插件极其丰富。
...
对于仓库里那些比较大的文件——几十兆的 Excel、设计稿、数据集——推荐用 Syncthing 做双向同步。
...
![CMUX 界面：左侧垂直侧边栏显示 workspace 列表、git 分支、PR 状态等信息](https://zj1123581321.com/posts/260628-agent-era-dev-anywhere/cmux-ui.jpg)
...
而且还有一个更重要的原因：**风控安全**。我在 [风控篇](https://mp.weixin.qq.com/s/Set_2-M-QP2xSAhyl1MonQ) 里详细分析过，手机 APP 能获取的传感器信息（GPS 定位、基站信息、系统时区等）远超你通过网络层面能伪装的范围。对于有风控策略的服务来说，在手机上装原生客户端等于主动把自己的物理位置交出去。所以我在手机上只使用 Claude 和 ChatGPT 的 PWA 网页版（PWA 就是把网页"安装"到手机桌面上，打开后看起来像原生 APP，但本质上还是浏览器在跑，不会获取手机传感器信息），**所有和 Coding Agent 的交互都走 HAPI**——手机只是一个显示终端，不会暴露任何手机端的传感器信息。
...
![HAPI 手机端界面：远程控制 Claude Code Agent 的实际交互画面](https://zj1123581321.com/posts/260628-agent-era-dev-anywhere/hapi-web-ui.png)
...
Tailscale 做的事情用大白话说就是：**不管你的设备在哪个物理网络上——家里的 WiFi、公司的网络、咖啡厅的 WiFi、手机的 4G——它都能把这些设备拉进同一个虚拟的局域网里。**
...
但手机上有个麻烦：Tailscale 作为 VPN 运行，手机上同一时间只能开一个 VPN 应用。如果你同时在用别的网络工具，就会冲突。这时候你就需要一个不依赖 VPN 的备选方案——Cloudflare Tunnel。
...
虽然 HAPI 本身有登录鉴权，但如果你希望更保险，可以再叠一层 Cloudflare Access。
...
具体的备份策略（3-2-1 原则、PVE 镜像互通、115 网盘等），我在 [硬件篇](https://mp.weixin.qq.com/s/I2ksdFm_jbNqoTPD_QSuuw) 里详细写过，这里不重复展开。
...
![Beszel 监控仪表盘：一个页面查看所有设备的运行状态](https://zj1123581321.com/posts/260628-agent-era-dev-anywhere/beszel-dashboard.png)
...
![随时、随地、随设备](https://zj1123581321.com/posts/260628-agent-era-dev-anywhere/anywhere-concept.jpg)
...
> 以下内容面向 Coding Agent（Claude Code / Codex 等）。如果你是人类读者，可以跳过这部分——把博客原文链接（ https://zj1123581321.com/posts/260628-agent-era-dev-anywhere/ ）发给你的 Agent，它会自己来读这一节。

### 服务器环境搭建

| 组件 | 参考文档 | 踩坑点 |
|------|---------|--------|
| PVE | Proxmox VE Installation | 安装后默认启用企业 apt 源（需订阅），必须改成 `pve-no-subscription` 源，否则 `apt update` 会报错 |
| Ubuntu 24.04 LTS | Ubuntu Server Guide | 选无桌面版（Server），虚拟机建议分配 16G+ 内存 |
...
| 组件 | 参考文档 | 踩坑点 |
|------|---------|--------|
| Tailscale | 官方下载页 | 手机上作为 VPN 运行，与其他 VPN/代理应用冲突。可通过 Exit Node 让特定设备转发流量来缓解，但门槛较高 |
| Tailscale Peer Relay | Peer Relay Docs | 需要 Tailscale >= 1.86；配置命令 `tailscale set --relay-server-port=40000`；验证 `tailscale status \| grep peer-relay` |
| Cloudflare Tunnel | Get Started | 前提：域名 NS 需指向 Cloudflare；`cloudflared` 建议配为 systemd 服务自启动（`sudo cloudflared service install`）；config.yml 中 url 写 `http://localhost:<HAPI Hub 端口>` |
| Cloudflare Access | Access Policies | 在 Zero Trust Dashboard 中配置；Telegram Mini App 要求 HTTPS，Tunnel 自带 HTTPS 正好满足 |

### 客户端与工具

| 工具 | 参考文档 | 踩坑点 |
|------|---------|--------|
| VS Code Remote SSH | 官方文档 | 先在 `~/.ssh/config` 配好 Host 免密登录 |
| Syncthing | Getting Started | 必须配 `.stignore` 排除 `node_modules`、`.git` 等目录；首次同步大仓库时暂停 Agent 写入避免冲突 |
| cc-clip | GitHub | 安装后 `cc-clip setup <SSH Host名>` 全自动部署；需要远程有 `xclip` 或 `wl-paste`；出问题用 `cc-clip doctor --host <host>` 诊断；使用用户级 token 认证，不适合共享服务器 |
| CMUX | GitHub | `brew tap manaflow-ai/cmux && brew install --cask cmux`；仅 macOS |
| HAPI | 安装文档 | npm 全局安装建议用官方 registry（国内镜像可能缺平台包）；远程模式用 `hapi hub --relay` + `hapi runner start`；Telegram 集成需设 `TELEGRAM_BOT_TOKEN` 和 `HAPI_PUBLIC_URL`（需 HTTPS） |
| Beszel | Getting Started | Agent 和 Hub 通过 SSH 通信，首次添加需配 SSH Key |
