...
tags:
  ...
  - Herdr
  - 工具
description: 从 tmux + HAPI 到 Herdr + HerdWeb，一次手机远程开发体系的全面升级：角标 API 解决状态盲区、七项针对手机屏幕的交互优化、Mosh 协议启发的弱网方案，以及选型背后的取舍逻辑。
---

> 本文是「Agent 不下班」系列的手机方案升级篇。基础架构怎么搭，见 [Agent 不下班：随时随地随设备的开发体系](/posts/260628-agent-era-dev-anywhere/)；多 Agent 怎么调度，见 [将军赶路不追小兔](/posts/260809-multi-agent-scheduling-architecture/)。这篇聊的是：整套体系切换到 Herdr 之后，手机端的开发体验怎么跟着升级。
>
...
在 [Agent 不下班](/posts/260628-agent-era-dev-anywhere/) 那篇文章里，我推荐的手机方案是 **HAPI + tmux**，网络走 Tailscale 或 Cloudflare Tunnel。用了一段时间之后，出现了两个越来越明显的问题。

### 裂缝一：tmux 的状态盲区

在 [多 Agent 调度](/posts/260809-multi-agent-scheduling-architecture/) 那篇里我提过，我现在的调度思路是**主脑派任务卡**——主脑负责规划和拆任务，执行层在后台自主执行。
...
![Herdr 角标状态：一眼看清每个 Agent 的当前状态——done、idle、working、等待回复](herdr-badge-status.png)
...
![HerdWeb 手机端实际界面：底部八个一级操作按钮，Herdr 会话完整渲染，右上角可切换服务器](herdweb-mobile-ui.jpg)
...
它本质上是一个个人部署的工具，鉴权这件事应该交给更专业、更成熟的基础设施来处理——就像上一篇文章里介绍过的 [Tailscale](/posts/260628-agent-era-dev-anywhere/#tailscale把所有设备拉进同一个安全的虚拟-wifi) 或 [Cloudflare Tunnel + Access](/posts/260628-agent-era-dev-anywhere/#cloudflare-tunnel--access手机访问-hapi-的方案)。
...
- 鉴权配置方式与 [Agent 不下班 1.0](/posts/260628-agent-era-dev-anywhere/) 中的网络层完全相同
