...
至于执行层——以当前模型厂的迭代速度，我觉得都问题不大。我现在的主力执行层已经切到 GPT-5.6-sol Luna xHigh 模式，7 天大概有 300 亿 Token 可用，非常充裕。当前一线的模型，放到一两年前都比那个时代最好的编程助手强很多。它们只是欠缺全局视野和大局观，但如果任务卡拆得到位，执行层已经是一个基本被解决的问题。**所以问题的难点全部集中在了主脑上。**
...
我的经验值是：**每次提交控制在约 800 行代码**，上限不超过 3,500 行。这样负责审查的 Agent 能在它的"智力甜点区间"工作。
...
每周我都会针对过去的开发记录，调最好的模型做一次复盘。会结合 Claude Code 和 Codex 的官方最佳实践，看当前和 Agent 的协作过程中还有哪些卡点、可以提升的地方。
...
反推出来：**优先使用每个对话窗口的前 20% 空间**，那是 Agent 的"智力甜点区间"。知道什么时候该把当前对话里的关键信息总结成一份文档，然后开一个全新的对话让 Agent "满血复活"——这是一种需要培养的直觉。
